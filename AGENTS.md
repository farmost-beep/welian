...
- 文案、配置、阈值、feature flag 等通过后端 `/ai/config` 或 KV 驱动，前端只做展示
- 新功能优先评估"能否通过后端配置实现"，能则不改前端代码
- 前端改动只在以下情况发生：新 UI 组件、新页面、交互逻辑变更
- 前端发布成本高（需微信审核），后端发布成本低（wrangler deploy 秒级）
...
1. 不自动点赞、不自动加好友
2. 不自动发送私聊消息——私聊代表一对一的私人关系，用心不可自动化
3. 群聊自动回复需满足 SPEC 6.3 全部条件（群白名单+触发条件+频率限制+可关闭+标注AI）
4. 不对陪伴型关系做 ROI/排序/冷却
5. 不做幸福评分
6. 不编造数据
7. 不做关系数据变现
8. 不做"附近的人"/陌生人社交
