#!/usr/bin/env node
/**
 * Cloudflare Pages deployment script for Welian.
 *
 * Uses the Cloudflare Pages API directly with BLAKE3 hashing
 * (same as wrangler) to bypass Node.js fetch proxy issues with Clash Verge.
 *
 * Usage: node scripts/deploy.mjs
 */

import { ProxyAgent, setGlobalDispatcher, fetch, FormData } from '/opt/homebrew/lib/node_modules/wrangler/node_modules/undici';
...
// Proxy setup (Clash Verge runs on 127.0.0.1:7897)
...
// BLAKE3 hash: blake3(base64(content) + extension).toString('hex').slice(0, 32)
...
async function main() {
  ...
  // Step 2: Check which hashes are missing
